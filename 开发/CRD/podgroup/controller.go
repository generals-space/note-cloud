...
import (
	...
	crdV1 "podgroup/pkg/apis/testgroup/v1"
	crdClientset "podgroup/pkg/client/clientset/versioned"
	crdScheme "podgroup/pkg/client/clientset/versioned/scheme"
	crdInformers "podgroup/pkg/client/informers/externalversions/testgroup/v1"
	crdListers "podgroup/pkg/client/listers/testgroup/v1"
)
...
type Controller struct {
	...
	podGroupClientset crdClientset.Interface

	podGroupLister crdListers.PodGroupLister
	podGroupSynced cgCache.InformerSynced
...
func NewController(
	kubeclientset cgKuber.Interface,
	podGroupClientset crdClientset.Interface,
	podGroupInformer crdInformers.PodGroupInformer) *Controller {
	...
	eventBroadcaster.StartRecordingToSink(&cgCorev1.EventSinkImpl{Interface: kubeclientset.CoreV1().Events("")})
	recorder := eventBroadcaster.NewRecorder(cgScheme.Scheme, apiCorev1.EventSource{Component: controllerAgentName})
	...
	controller := &Controller{
		...
		workqueue:         cgWorkqueue.NewNamedRateLimitingQueue(cgWorkqueue.DefaultControllerRateLimiter(), "PodGroups"),
		recorder:          recorder,
	}
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	if ok := cgCache.WaitForCacheSync(stopCh, c.podGroupSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		...
		apimRuntime.HandleError(fmt.Errorf("failed to list podGroup by: %s/%s", namespace, name))
		return err
	}
	...
	c.recorder.Event(podGroup, apiCorev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	return nil
}
